...
\title[a novel telephony network architecture]{\huge{On creating a novel, future
...
\frame {
	\frametitle{Scenario: Alice calls Bob}
	\begin{enumerate}
		\item Both parties have exchanged their $\text{Key}_{\text{public}}$ (once)\medskip
		\item Alice calls i.e. $1234$ from her SIP phone, where $g(1234)$:=\texttt{Bob}\medskip
		\item Since \texttt{Bob} has a registered public key, $h(\text{\texttt{Bob}})$ returns $\text{Key}_{\text{Bob,public}}$\medskip
		\item Alice looks up $SHA_{256}(\text{Key}_{\text{Bob,public}})$ by applying to $f$ in the DHT\medskip
		\item Alice receives $[\text{IP}_{\text{Bob,public}}, \text{Port}_{\text{Bob,public}}]$\medskip
		\item Alice opens a direct and $\text{Key}_{\text{Bob,public}}$-encrypted connection to Bob\medskip
		\item Bob accepts the connection from Alice, delivers a notification to the SIP phones and both transfers their encrypted voice data
	\end{enumerate}
}
